Vec2 bldrVel = new Vec2(0,0);
Vec2 bldrPos = new Vec2(480, 330);
float bldrSpeed = 40;
float bldrRadius = 32;
...
public class WaterParticle {
  ...
  void update(){
    ...
    if (loc.distanceTo(bldrPos) < (bldrRadius+r)){
      Vec2 normal = (loc.minus(bldrPos)).normalized();
      loc = bldrPos.plus(normal.times(bldrRadius+r).times(1.01));
      Vec2 velNormal = normal.times(dot(vel,normal));
      vel.add(bldrVel); // possible answer for adding momentum
      //vel.add(projAB(bldrVel,normal)); // professor's solution
